...
library AccountingMath {
    ...
    function withAtlasSurcharge(uint256 amount) internal pure returns (uint256 adjustedAmount) {
        adjustedAmount = amount * (_SURCHARGE_SCALE + _ATLAS_SURCHARGE_RATE) / _SURCHARGE_SCALE;
    }
    ...
    // gets the Atlas surcharge from an unadjusted amount
    function getNetAtlasSurchargeFromBundlerSurcharge(uint256 claims) internal pure returns (uint256 surcharge) {
        surcharge = claims * _ATLAS_SURCHARGE_RATE / _BUNDLER_SURCHARGE_RATE;
    }

    // gets the Atlas surcharge from an unadjusted amount
    function extractAtlasSurcharge(uint256 amount) internal pure returns (uint256 surcharge) {
        surcharge = amount * _ATLAS_SURCHARGE_RATE / _SURCHARGE_SCALE;
    }

    function extractBundlerSurcharge(uint256 amount) internal pure returns (uint256 surcharge) {
        surcharge = amount * _BUNDLER_SURCHARGE_RATE / _SURCHARGE_SCALE;
    }
}
